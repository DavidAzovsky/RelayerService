...
describe("PriceFeed Contract", function () {
  ...
  before(async function () {
    ...
    targetTokenInstanceOne = await deployTargetToken(
      "TargetTokenOne",
      "TONE",
      receiverForwarder.address
    );
    targetTokenInstanceTwo = await deployTargetToken(
      "TargetTokenTwo",
      "TTWO",
      receiverForwarder.address
    );
    targetTokenInstanceThree = await deployTargetToken(
      "TargetTokenThree",
      "TTHREE",
      receiverForwarder.address
    );
  ...
  it("OwnerShip & Config", async function () {
    ...
    await receiverForwarder
      .connect(owner)
      .setTargetContractStatus(
        [
          targetTokenInstanceOne.address,
          targetTokenInstanceTwo.address,
          targetTokenInstanceThree.address,
        ],
        true
      );

    expect(
      await receiverForwarder.getTargetContractStatus(
        targetTokenInstanceOne.address
      )
    ).to.equal(true);
    expect(
    ...
    expect(await receiverForwarder.getFunctionStatus("0xa9059cbb")).to.equal(
      true
    );
  });

  it("Ownership pause & unpause", async function () {
    await expect(receiverForwarder.connect(caller).pause()).to.be.reverted;
    await expect(receiverForwarder.connect(caller).unpause()).to.be.reverted;
  });

  it("Execute Transfer Revert if Paused", async function () {
    await receiverForwarder.connect(owner).pause();

    const tokenAmount = parseEther("2");

    await targetTokenInstanceOne
      .connect(owner)
      .mint(caller.address, parseEther("10"));
    const beforeUserBalance = await targetTokenInstanceOne.balanceOf(
      caller.address
    );
    ...
    await expect(
      receiverForwarder.connect(executor).execute([ForwardRequest], [signature])
    ).to.be.reverted;
  });

  it("Execute Transfer", async function () {
    await receiverForwarder.connect(owner).unpause();
    ...
    const afterUserBalance = await targetTokenInstanceOne.balanceOf(
      caller.address
    );
    const receiverBalance = await targetTokenInstanceOne.balanceOf(
      receiver.address
    );
  ...
  async function deployReceiverForwarder() {
    let ReceiverForwarderArtifacts = await ethers.getContractFactory(
      "ReceiverForwarder"
    );
  ...
  async function getTransferCallData(address: string, amount: BigNumber) {
    ...
    return token_itf.interface.encodeFunctionData("transfer", [
      address,
      amount,
    ]);
  }
